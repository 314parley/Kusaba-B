	...
	*/
	...
	function makereq() {
		...
		if(empty($_GET['pid']) || !is_numeric($_GET['pid'])) {
			$tpl_page .= '<h3>No post ID specified.</h3>';
			return;
		}
		if(empty($_POST)) {
			$tpl_page .= '<form action="' . KU_WEBPATH . '/manage_page.php?action=makereq&pid=' . $_GET['pid'] . '" method="post">
				<div>
					<input type="text" name="reason" />
					<label for="reason">Reason for banning</label>
				</div>
				<div>
					<input type="submit" value="Request Ban" />
				</div>
			</form>';
		}
		else {
			$exist_check = $tc_db->GetOne("SELECT * FROM `" . KU_DBPREFIX . "ban_requests` WHERE `post` = " . $_GET['pid'] . " LIMIT 1");
			if(!empty($exist_check[0][0])) {
				$tpl_page = '<h3>That post was <a href="manage_page.php?action=request&id=' . $exist_check[0][0] . '">already submitted</a> for banning.</h3>';
				return;
			}
			$post_info = $tc_db->GetOne("SELECT * FROM `" . KU_DBPREFIX . "posts` WHERE `id` = " . $_GET['pid']);
			$janitor_id = $tc_db->GetOne("SELECT `id` FROM `" . KU_DBPREFIX . "staff` WHERE `username` = " . $tc_db->qstr($_SESSION['manageusername']));
			$janitor_id = $janitor_id[0][0];
			if(empty($post_info[0][0])) {
				$tpl_page = '<h3>Post does not exist</h3>';
			}
			$tc_db->Execute("INSERT HIGH_PRIORITY INTO `" . KU_DBPREFIX . "ban_requests` ( `janitor` , `post` , `reason` ) VALUES ( " . $tc_db->qstr($janitor_id) . " , " . $tc_db->qstr($_GET['pid']) . " , " . $tc_db->qstr($_POST['reason']) . " )");
			$tpl_page = '<h3>Ban request submitted.</h3>';
		}
	}

	function mybanreqs() {
		global $tc_db, $tpl_page;
		$janitor = $tc_db->GetOne("SELECT `id` FROM `" . KU_DBPREFIX . "staff` WHERE `username` = " . $tc_db->qstr($_SESSION['manageusername']));
		$janitor = $janitor[0][0];
		$requests = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "ban_requests` WHERE `janitor` = " . $janitor);
		if(!empty($requests[0][0]))
		{
			$tpl_page = '<h2>' . _gettext('My Ban Requests') . '</h2>';
			$tpl_page .= '<ul>';
			foreach($requests as $request)
			{
				$tpl_page .= '
				<li style="margin-left:25px;"><a href="manage_page.php?action=request&id=' . $request[0] . '"' . ($request['status'] == 0 ? ' style="font-weight:bold; color:#a60000;"' : '') . '>' . _gettext('Request #' . $request[0]) . '</a></li>';
			}
			$tpl_page .= '</ul>';
		}
		else
		{
			$tpl_page .= '<h3>' . _gettext('No ban requests') . '</h3>';
		}
	}
	...
	function banreqs() {
		...
		$this->ModeratorsOnly();
		$tc_db->debug = true;
		$requests = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "ban_requests`");
		var_dump($requests[0]);
		if(!empty($requests[0][0]))
		{
			$tpl_page = '<h2>' . _gettext('Ban Requests') . '</h2>';
			$tpl_page .= '<ul>';
			foreach($requests as $request)
			{
				$tpl_page .= '
	...
	function request() {
		...
		if(!isset($_GET['id']) || !is_numeric($_GET['id'])) {
			$tpl_page .= '<h3>' . _gettext('No ID specified') . '</h3>';
		}
		elseif(isset($_GET['changestat'])) {
			$this->ModeratorsOnly();
			$ban_request = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "ban_requests` WHERE `id` = " . $_GET['id'] . " LIMIT 1");
			if($ban_request[0]['status'] != 0) {
				$tpl_page = '<h3>' . _gettext('That ban request was already closed.') . '</h3>';
				return;
			}
			
			$mod_id = $tc_db->GetOne("SELECT `id` FROM `" . KU_DBPREFIX . "staff` WHERE `username` = " . $tc_db->qstr($_SESSION['manageusername']));
			$mod_id = (int)$mod_id[0][0];
			if($_GET['changestat'] == 1) {
				$ban_request = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "ban_requests` WHERE `id` = " . $_GET['id'] . " LIMIT 1");
				$ban_request = $ban_request[0];
				$post = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "posts` WHERE `id` = " . $ban_request['post'] . " LIMIT 1");
				$post = $post[0];
				$board = $tc_db->GetOne("SELECT `name` FROM `" . KU_DBPREFIX . "boards` WHERE `id` = " . $post['boardid'] . " LIMIT 1");
				
				$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "ban_requests` SET `status` = 1, `moderator` = " . $mod_id . " WHERE `id` = " . $_GET['id']);
				management_addlogentry(_gettext('Closed ban request ' . $_GET['id'] . ' with ban.'));
				$ban_info = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "ban_requests` WHERE `id` = " . $_GET['id'] . " LIMIT 1");
				$tpl_page .= '<h3>'. _gettext('Closed ban request ' . $_GET['id'] . '. You will be redirected to the ban screen.') . '</h3>
				<script type="text/javascript">
					setTimeout(function(){ this.location = "' . KU_WEBPATH . '/manage_page.php?action=bans&banboard=' . $board[0]['name'] . '&banpost=' . $post['id'] . '"; }, 2500);
				</script>';
			}
			else {
				$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "ban_requests` SET `status` = 2, `moderator` = " . $mod_id . " WHERE `id` = " . $_GET['id'] . " LIMIT 1");
				management_addlogentry(_gettext('Closed ban request ' . $_GET['id'] . ' without ban.'));
				$tpl_page .= '<h3>'. _gettext('Closed ban request ' . $_GET['id'] . ' without ban.') . '</h3>
				<script type="text/javascript">
					setTimeout(function(){ top.location = "' . KU_WEBPATH . '/manage.php"; }, 2500);
				</script>';
			}
		}
		else {
			$ban_request = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "ban_requests` WHERE `id` = " . $_GET['id'] . " LIMIT 1");
			if(empty($ban_request[0][0])) {
				$tpl_page = '<h3>That ban request does not exist.</h3>';
			}
			else {
				$ban_request = $ban_request[0];
				$post = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "posts` WHERE `id` = " . $ban_request['post'] . " LIMIT 1");
				$post = $post[0];
				$board = $tc_db->GetOne("SELECT `name` FROM `" . KU_DBPREFIX . "boards` WHERE `id` = " . $post['boardid'] . " LIMIT 1");
				$janitor = $tc_db->GetAll("SELECT `username` FROM `" . KU_DBPREFIX . "staff` WHERE `id` = " . $ban_request['janitor'] . " LIMIT 1");
				$moderator = @$tc_db->GetAll("SELECT `username` FROM `" . KU_DBPREFIX . "staff` WHERE `id` = " . $ban_request['moderator'] . " LIMIT 1");
				$tpl_page .= '<h2>Ban Request #' . $_GET['id'] . '</h2>';

				if($ban_request['status'] == 0) {
				$tpl_page .= '<ul style="font-size:13px; font-family:Tahoma, sans-serif;">
					<li><a href="' . KU_WEBPATH . '/' . $board['name'] . '/res/' . ($post['parentid'] < 1 ? $post['id'] : $post['parentid']) . '.html#' . $post['id'] . '" style="text-decoration:none;" target="_blank">Post #</a>' . $ban_request['post'] . ' on <a href="' . KU_WEBPATH . '/' . $board['name'] . '" target="_blank">/' . $board['name'] . '/</a></li>
					<li>Made by <strong>' . (empty($post['name']) ? 'Anonymous' : stripslashes($post['name'])) . '</strong>' . (!empty($post['tripcode']) ? '!' . stripslashes($post['tripcode']) : '') . '</li>
					<li>IP Address: ' . md5_decrypt($post['ip'], KU_RANDOMSEED) . '</li>
				</ul>';
				$tpl_page .= '<h2>Post Content</h2>
				<blockquote>
					<table border="0" cellpadding="3" cellspacing="2">
						<tr>
							<td>
								<a href="" style="border:none; text-decoration:none;" target="_blank"><img src="' . KU_WEBPATH . '/' . $board['name'] . '/thumb/' . $post['file'] . 's.' . $post['file_type'] . '" alt="" title="' . $post['file_original'] . '.' . $post['file_type'] . '" style="border:none; text-decoration:none;" /></a>
							</td>
							<td>
								<span style="color:#006400;"><span style="font-weight:bold;">' . (empty($post['name']) ? 'Anonymous' : stripslashes($post['name'])) . '</span>' . (!empty($post['tripcode']) ? '!' . $post['tripcode'] : '') . '</span> posted at ' . date('F j, Y, g:i a', $post['timestamp']) . ':<br />
								<p style="word-wrap:break-word;">' . (empty($post['message']) ? '<em>[blank post]</em>' : stripslashes($post['message'])) . '</p>
							</td>
						</tr>
					</table>
				</blockquote>';
				...
				$tpl_page .= '
				<h2>Reason</h2>
				<strong style="font-family:Tahoma, sans-serif; color:#000053;">' . stripslashes($janitor[0][0]) . '</strong> filed this ban request with the following reason:
				<blockquote style="word-wrap:break-word;">
					' . stripslashes($ban_request['reason']) . '
				</blockquote>
				<h2>Ban Request Status</h2>
				';
				switch($ban_request['status'])
				{
					case 0:
						$tpl_page .= '<span style="font-weight:bold; color:#950000;">Open / Unhandled.</span>';
						break;
					case 1:
						$tpl_page .= '<span style="font-weight:bold; color:#009500;">Closed and Banned</span> by <span style="font-weight:bold; color:#950000;">' . $moderator[0]['username'] . '</span>';
						break;
					case 2:
						$tpl_page .= '<span style="font-weight:bold; color:#000095;">Closed; NOT Banned</span> by <span style="font-weight:bold; color:#950000;">' . $moderator[0]['username'] . '</span>';
						break;
					default:
						$tpl_page .= '<span style="font-weight:bold; color:#000000;">Unknown.</span>';
						break;
				}
				if($ban_request['status'] == 0
					&& ($this->currentUserIsModerator()
						|| $this->currentUserIsAdministrator())) {
				$tpl_page .= '<br /><br /><h2>Moderation</h2>
				<a href="' . KU_WEBPATH . '/manage_page.php?action=request&id=' . $_GET['id'] . '&changestat=1">Close Request and Ban User</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<a href="' . KU_WEBPATH . '/manage_page.php?action=request&id=' . $_GET['id'] . '&changestat=2">Close Request; Do NOT Ban User</a>
				';
				}
			}
		}
	...
	function purgereqs() {
		...
		$tc_db->Execute('DELETE FROM `' . KU_DBPREFIX . 'ban_requests`');
		$tpl_page = '<h3>All ban requests purged.</h3>';
	}

...
if (!$manage_class->ValidateSession(true)) {
	...
	// For compatibility with my other mods,
	...
	$tc_db->debug = false;
	if ($manage_class->CurrentUserIsAdministrator() || $manage_class->CurrentUserIsModerator()) {
		$open_requests = $tc_db->GetAll("SELECT HIGH_PRIORITY COUNT(*) FROM `" . KU_DBPREFIX . "ban_requests` WHERE `status` = '0'");
		var_dump($open_requests);
		$tpl_links .= '<li><a href="manage_page.php?action=banreqs">' . _gettext('Ban Requests') . '[' . $open_requests[0][0] . ']</a></li>';
		if($manage_class->CurrentUserIsAdministrator()) {
			$tpl_links .= '<li><a href="manage_page.php?action=purgereqs">' . _gettext('Purge All Requests') . '</a></li>';
		}
	}
	else {
		$uid = $tc_db->GetOne("SELECT `id` FROM `" . KU_DBPREFIX . "staff` WHERE `username` = " . $tc_db->qstr($_SESSION['manageusername']));
		$open_requests = $tc_db->GetAll('SELECT HIGH_PRIORITY COUNT(*) FROM `' . KU_DBPREFIX . 'ban_requests` WHERE `status` = 0 AND `janitor` = ' . $uid[0][0]);
		$tpl_links .= '<li><a href="manage_page.php?action=mybanreqs">' . _gettext('My Requests [' . $open_requests[0][0] . ']') . '</a></li>';		
	}
	$tpl_links .= '</ul></div>';
